...
Supplier management                                      3       1      0       0              2
Workforce and talent management                          4       4      0       0              0
Availability management                                  3       2      0       0              1
...
**component dependency model**  proven
...
**architecture standards register**  proven
...
**change impact assessment**  proven
...
**improvement measurement**  proven
...
**improvement review report**  proven
...
**improvement business case**  no real input
...
**confidentiality obligations register**  proven
...
**security incident report**  no real input

```
shape: incidents with severity and timeline
instantiated on: none
-> cases (duckdb: one row per case from the event rows)
-> workbook name=security_incidents (openpyxl.Workbook.save)
...
**frequently discussed topics**  proven
...
**training records**  no real input
...
**satisfaction report**  no real input
...
**chargeback report**  no real input
...
**third-party obligations register**  proven
...
**supplier register**  no real input

```
shape: suppliers and what they supply
instantiated on: none
-> events (P6 rows as a relation)
...
**supplier performance report**  no real input
...
**roles and responsibilities**  proven
...
**availability requirements**  proven
...
**availability plan**  no real input

```
shape: planned measures to keep the service available
instantiated on: none
-> cases (duckdb: one row per case from the event rows)
...
-> change model
proofs/out/itil/change-enablement/change-model/change_model.bpmn       sha256 166bbb4144b5a68850328b79b7388ac030a78e9a76c08820422136062d598113  registered
...
**change authority matrix**  proven
...
**post-implementation review**  proven
...
**license compliance report**  no real input

```
shape: entitlements against installations
instantiated on: none
-> events (P6 rows as a relation)
...
**release schedule**  proven

```
shape: releases by date
instantiated on: TSC minutes sections mentioning release
-> minutes_sections (markdown.markdown; lxml.html: heading sections and their list items)
-> filter where=[["item", "matches", "(?i)release|v[0-9]+\\b"]] (duckdb WHERE)
-> select fields=["meeting", "section", "item", "link"] (duckdb SELECT)
-> workbook name=release_schedule (openpyxl.Workbook.save)
-> release schedule
proofs/out/itil/release-management/release-schedule/release_schedule.xlsx sha256 b7f6fa5a330a135cda6d279a7f982c6d3385aa22473d113627e0148cc522f5bc  registered
shows: rows 13; first {"meeting": "tsc-2024-01-10", "section": "Announcements", "item": "Richard, Ulises did a release for Node.js 20", "link": ""}
```

**release readiness checklist**  proven
...
**configuration item register**  proven
...
**configuration verification report**  proven
...
**business impact analysis**  proven
...
**requirements traceability matrix**  proven
...
-> service blueprint
proofs/out/itil/service-design/service-blueprint/blueprint.bpmn        sha256 bff8566093ca1992bd9aeefe5ab597f4381897b80ddf30fa664c68d3f166e3a2  registered
...
**infrastructure inventory**  proven
...
**version records**  proven

```
shape: versions mentioned and when
instantiated on: TSC minutes items naming a version
-> minutes_sections (markdown.markdown; lxml.html: heading sections and their list items)
-> filter where=[["item", "matches", "\\b(v[0-9]{1,2}|Node\\.js [0-9]{1,2}|[0-9]+\\.[0-9]+\\.[0-9]+)\\b"]] (duckdb WHERE)
-> select fields=["meeting", "item", "link"] (duckdb SELECT)
-> workbook name=versions (openpyxl.Workbook.save)
-> version records
proofs/out/itil/software-development-and-management/version-records/versions.xlsx sha256 347c9f615b3d7a300d95667077057729afaff0418587411211fca4a41257d865  registered
shows: rows 4; first {"meeting": "tsc-2024-01-10", "item": "Richard, Ulises did a release for Node.js 20", "link": ""}
```

**verification report**  proven
...
## Variations: the numbered proofs

```
deliverable files 93: reproduced 0, registered now 93, changed 0
```
...
```
proofs/out/P1/usc5-552-doj/facts.jsonl                     sha256 08d2fc2fff7c11e7810c7658fa2f0deac188bae1cffa32603a21a903556991b3  registered
...
```
proofs/out/P1/nodejs-tsc-charter/facts.jsonl               sha256 693daac33fdd38aab8a0a887edabdc6615cdbb30e72cc2127134d09a02d0cb47  registered
...
```
proofs/out/P1/nodejs-governance/facts.jsonl                sha256 ce60a372408888a0e047ec3e23085bd97dae2db11bb50114ea565af5b20fba85  registered
...
```
proofs/out/P2/usc5-552-doj/ordered_steps.json              sha256 58a3c227a23125b3d645f2ea62e9be716b6e2987cdc40b837987c4bbfcf5c70f  registered
proofs/out/P2/usc5-552-doj/manifest.json                   sha256 117f82af151b14b539f97097097e455d89f4a96e31958b89c948c9014c6e5f2c  registered
...
```
proofs/out/P2/nodejs-tsc-charter/ordered_steps.json        sha256 f0bbb34105387719734233f7ea1ed88b41b00c6feeac7fa600dd620d9c637dc1  registered
proofs/out/P2/nodejs-tsc-charter/manifest.json             sha256 859cba40cf774897b4a323396202d6af2a210389592519f7e53e66320c381bd9  registered
shows: consistent: True; cycle: []
...
```
proofs/out/P2/nodejs-governance/ordered_steps.json         sha256 46fdd991c2c12639b557c1cb52000915cf18ec3782a34c6d5f18b1b9caf9fbce  registered
proofs/out/P2/nodejs-governance/manifest.json              sha256 6d165ccb2a474de1fcc29237a5e744c45ff40c67b41d8265e70f21b0fcff4048  registered
shows: consistent: True; cycle: []
...
```
proofs/out/P3/usc5-552-doj/anchor_dates.jsonl              sha256 f835e3f6bbabc0adcc8e3d0e44673fd9ded73241250429327fd07ab7dd3a6da3  registered
...
```
proofs/out/P3/nodejs-tsc-charter/anchor_dates.jsonl        sha256 0fa0ca1b1eb87b16b47381b86b335b0c3877c51756a88d3a3b339bfeba475de0  registered
shows: "one year" -> TIMEX3 type="DURATION" value="P1Y"
```
...
```
proofs/out/P4/usc5-552-doj/policy.jdm.json                 sha256 c8930334e2ce3d584228d60d760abda5f6367efc7df80ede11d3c75c5ef54ca2  registered
...
```
proofs/out/P4/nodejs-tsc-charter/policy.jdm.json           sha256 8d0dd9917df562ecac23b7c06d73905ed2aa7c264eb6df19d0773f6bfd28457a  registered
...
```
proofs/out/P5/usc5-552-doj/process.bpmn                    sha256 c5a466c1419b16b7cb6ecaf2f2385b683d840857e2322b912818e7d3bc481821  registered
...
```
proofs/out/P5/nodejs-governance/process.bpmn               sha256 ad2e5871899b598d98c1d810312b9c93c5bc38dbd86ccfb88138f55b92dde727  registered
...
```
proofs/out/P6/receipt-csv/facts.lp                         sha256 61ceee89f87cc39c2de45474c2e2f3c093e297e410489314fdd60798722e4126  registered
...
```
proofs/out/P6/tsc-voting-members/facts.lp                  sha256 02494e198a159fbb0a4553a9f23ed650d027b0e94a838828089fba92f423e839  registered
...
```
proofs/out/P7/nodejs-tsc-minutes/facts.jsonl               sha256 a8aa9ada6ba446dbd6136cf9cdfd1d9e4c352ec7abda6432dd469f44a7ca9758  registered
...
```
proofs/out/P8/receipt-xes/process.pnml                     sha256 746b108c4d9bbd566f3d7bab818cd265fc6357027d6313d1a0de904a7347720d  registered
proofs/out/P8/receipt-xes/process.bpmn                     sha256 66fa1e38525674151fce339caf0dcbd023ce7520931c990f9bdc4402dea2de46  registered
...
```
proofs/out/P9/receipt-xes/conformance.xlsx                 sha256 339533b73457ffcd2df529f6452ce6e728f35ebc244dbbc36f394a8d8912f007  registered
...
```
proofs/out/P10/nodejs-governance+nodejs-tsc-minutes/tagged_steps.xlsx sha256 ef1b2d2ffe35418645977ad97f740a5729b06ae1dbcf9d59377d40fb2bc371af  registered
proofs/out/P10/nodejs-governance+nodejs-tsc-minutes/tags.jsonl sha256 00216f1e1e3eb38cb74e11d3478787ffb2cc219d093277e9fb74dec5e2389f2d  registered
...
```
proofs/out/P11/nodejs-governance/measured_steps.xlsx       sha256 1cf193eb87f0f788f04a95942b3ff729655bba8ccf0c0c1c50e62c63c1af03d0  registered
shows: be: 40
shows: add: 16
shows: share: 13
shows: time: 11
...
```
proofs/out/P11/nodejs-tsc-charter/measured_steps.xlsx      sha256 2bdf39eabedc9f5d2afd5a8b04de0dc93a29dd6d4afdf1f4785c1e2802b0ce51  registered
shows: have: 31
shows: establish: 30
shows: direct: 15
shows: meet: 9
...
```
proofs/out/P12/receipt-xes+receipt-csv/measured_activities.xlsx sha256 544bda186c506df777a499206424b268bab52501000cbcaeed1e88fd12a754ad  registered
...
```
proofs/out/P13/charter+roster+minutes/policy.jdm.json      sha256 e038135743787da6ec53bb05fbff39ff9be42b7ceba99398f9dcddb9c11428d9  registered
proofs/out/P13/charter+roster+minutes/decisions.xlsx       sha256 c333cd29051c0297a3d1cc188f850d15089879ada856e96b54f3f1a28d211a25  registered
...
```
proofs/out/P14/usc5-552-doj/dependencies.pptx              sha256 36f04460d3c94f482f06c73a56d9591851905ed2d85faf64be32f2f21b421728  registered
...
```
proofs/out/P14/nodejs-governance/dependencies.pptx         sha256 349160c68b6d1a1f705620c05641124b9f58afdb0851b3fbb85235f05a5d501b  registered
...
```
proofs/out/P15/usc5-552-doj/ordered_steps.docx             sha256 6da847e325f9ebb56a3bf4981788ed3bee5bcd9802b839824930f7062e1d14d1  registered
...
```
proofs/out/P15/nodejs-governance/ordered_steps.docx        sha256 fa6b8164919d776b3a2e7d4463600d56f77e26b77dca32368e9428961b0ec53f  registered
...
```
proofs/out/P16/usc5-552-doj/facts.xlsx                     sha256 895cd74f2f48c34da51e07aea39b781af0b4b4d202e8e7b2bfeda3e1dcf6efd4  registered
...
```
proofs/out/P16/nodejs-tsc-charter/facts.xlsx               sha256 549f9290f6744b52f6dbcfb9cfa58a35163e8016fe24921ffd34e7b4b705e0eb  registered
...
```
proofs/out/P16/nodejs-governance/facts.xlsx                sha256 f9608a6e76ef2643940b1bc26d440e95e643d3175e171fc2870ae978838b7d46  registered
...
```
proofs/out/P17/all/digests.json                            sha256 db93bc9f9542c404a00ad0a18929394f9a2a5683a0f00d982390cf6a49f9d4e6  registered
shows: deliverables 92; digest af04fbf29b803b165092ccc5f47c68f87e979d9f0c2cd417db9944f5aa8a008b
```
...
```
proofs/out/P18/nodejs-governance+tsc-2024-01-17/deck.pptx  sha256 90af16c70fab459d27e65759c6140ff71226cae57a8bdb85e4d2603db8122bf1  registered
proofs/out/P18/nodejs-governance+tsc-2024-01-17/digest.json sha256 b2328db5e57553bf2e78baeae9d4ab96bf46dd621ad37e82c3c6a4c1f274c684  registered
...
```
proofs/out/P19/nodejs-governance/compare.json              sha256 a9a0e05c609bc913ab1654c8731d319fa7581ac6f8563f474f5dd438c1a8849c  registered
...
```
proofs/out/P20/nodejs-governance/conformance.xlsx          sha256 041de6a3fcad86eebbcf28c7f16a439361a9d756286c385713daeaf009d5deaa  registered
...
```
proofs/out/P21/receipt-csv/deadlines.xlsx                  sha256 f923fdc8cd38481fac9f426fbe55d23f8206bb1f8ad59528e14126229ae6cfd0  registered
...
```
proofs/out/P22/receipt-csv/by_department_and_channel.xlsx  sha256 4ad652e5f73e2286302b6b2d4f9ffadacf61c133a1b08ea5af02851f094b6968  registered
...
```
proofs/out/P23/receipt-xes/directly_follows.xlsx           sha256 07dfafdeaa64f3526c0aa177ab6de79d7df9bacc5f435e336a8555f56df04b68  registered
proofs/out/P23/receipt-xes/directly_follows.json           sha256 947a329a7817cca79e8268d567bdec66cf9268f03b103d7ae31dddb81dc9a904  registered
...
```
proofs/out/P24/receipt-xes+receipt-csv/dashboard.html      sha256 9e9eafed6a5d4e719bd43e1e7589d3645912c0db53f420d8670510903940d21a  registered
...
```
proofs/out/P25/usc5-552-doj/required_actions.docx          sha256 c910688d18800d40943d8e100fcf5282d4cdc95a25deac352fccfba5af418726  registered
...
```
proofs/out/P25/nodejs-tsc-charter/required_actions.docx    sha256 128db0ce380dbcf9f7b143d3ae340c9985eb4f31006bb583709a39beb76be08b  registered
shows: table rows 13
shows:  must open
...
```
proofs/out/P26/nodejs-tsc-charter+nodejs-tsc-minutes/tagged_steps.xlsx sha256 e3b7806a5238fa632f0882f023eb23689d930ef50a13fdb5479a572926e2411d  registered
proofs/out/P26/nodejs-tsc-charter+nodejs-tsc-minutes/tags.jsonl sha256 9f1e239f2364163034070c055de5d26c87b3539df330ecdbf717a1c603442a7f  registered
...
```
proofs/out/P27/receipt-xes+receipt-csv/conformance_by_group.xlsx sha256 70e4bb450df3a8387ebfbfa478c8fe9d593580672e9c091f4b53055dc26ab151  registered
...
```
proofs/out/P28/majority+charter-tags/decisions_with_discussion.xlsx sha256 4f0ba45d8e29fd3e3d7d32344965b1989cb7ddbc87db141a29c2a842105f5719  registered
...
```
proofs/out/P29/usc5-552-doj/process.executable.bpmn        sha256 adc5a48992f869cc8cc28100567b185eb30e8b41eff05dc8f44fdc5fbe56a217  registered
proofs/out/P29/usc5-552-doj/execution_trace.xlsx           sha256 adbd07ad97584717c888b7e88837f673eca2527bda3479912fb53718ca126212  registered
...
```
proofs/out/P29/nodejs-governance/process.executable.bpmn   sha256 2a172700c0bcfcf1273ecc38d00e2fb4966c2128b4a83fb8b9e6657ee1c378ae  registered
proofs/out/P29/nodejs-governance/execution_trace.xlsx      sha256 829d201465e891e466faca8422cdadc590f69b80fb4d34909df5e41e1420bea4  registered
...
```
proofs/out/P30/usc5-552-doj/roundtrip.json                 sha256 a8f11bdab6e62f5cc18f74624aa4bcb5840327bc674962224602cb3d4a412f39  registered
...
```
proofs/out/P31/majority/attendance.html                    sha256 80b764943b5eddb180a1575ec68af0999293a318c238ab8b31aae043c17586ec  registered
...
```
proofs/out/P32/charter+minutes/coverage_matrix.xlsx        sha256 c601b257f4de26d0cc1cf9e9d4e2a61f22f0e28bd3ce664bb9d58264ab32e8b4  registered
...
```
proofs/out/P33/nodejs-governance/never_discussed.xlsx      sha256 3d8f0878f0063c1ffc9412ff8560fb5f8cf3e9a573248ea8e7cbc9ac0a5b5229  registered
...
```
proofs/out/P34/receipt-xes/process.pnml                    sha256 e75e4c1e7f195eea21e50295697bf4f0d942476e98567f579c3cf9723ae91ebd  registered
proofs/out/P34/receipt-xes/soundness.json                  sha256 c651a83c5d8abd78fbe3f2c42f8f74e800c58da2f930822c8a1ee495c8005a82  registered
...
```
proofs/out/P35/usc5-552-doj/process.pnml                   sha256 3fc8ebba80e37cfb9bc1aa981e0ab07c17b4d3b1e729784f64a9dabf1fddc1b4  registered
proofs/out/P35/usc5-552-doj/soundness.json                 sha256 2fa0467fa5339ad4681c273c3cc2038ec4dab3255bda43076636c25fd6d04e7d  registered
...
```
proofs/out/P35/nodejs-governance/process.pnml              sha256 5b8649535023d4c0a6f1e5f7f5429cd523ba989d11618ab2c8ad85711f207063  registered
proofs/out/P35/nodejs-governance/soundness.json            sha256 0e408c69f24e2573396ab5a7cd2eadef2b020dc1533f6881924e04d6b7b86acd  registered
...
```
proofs/out/P36/receipt-xes/variants.xlsx                   sha256 fb2540bb2ab8ecfa30e1ecc13e60f6fcd390842b87b6ba7eab5fdc9512eed5f5  registered
...
```
proofs/out/P37/usc5-552-doj/facts.ttl                      sha256 b5d077ed4c3b1e4a4d2f20d8464b21ef13a0a154e5434a6f26ca1b891794d1cf  registered
proofs/out/P37/usc5-552-doj/sparql_required_actions.xlsx   sha256 20e20eae695f1d7ea439531fbcef9ec82a14023f0883d0d46c2044e7dc5b3340  registered
proofs/out/P37/usc5-552-doj/query.sparql                   sha256 f5bc9ca135b1412b919c5347bc162a0ca69b5aed1839ce6cda3226489d679715  registered
shows: triples 4051; obligatory events with an agent 68
shows: make by agency shall make
shows: state by agency shall separately state
shows: make by agency, in accordance with published rules, shall make
```

**nodejs-tsc-charter**

```
proofs/out/P37/nodejs-tsc-charter/facts.ttl                sha256 5e4f210151b30ecd7074e94fd924da280cfe7c50dd2d2f037ab26f969a2a7e5b  registered
proofs/out/P37/nodejs-tsc-charter/sparql_required_actions.xlsx sha256 aff33c5ad3a210af359135a3c17e7583fb6b49e516ca2561d0152951ae185e4e  registered
proofs/out/P37/nodejs-tsc-charter/query.sparql             sha256 f5bc9ca135b1412b919c5347bc162a0ca69b5aed1839ce6cda3226489d679715  registered
shows: triples 735; obligatory events with an agent 5
shows: have by TSC must have
shows: meet by TSC shall meet
shows: hold by TSC shall hold
```
...
```
proofs/out/P38/usc5-552-doj/cypher_results.xlsx            sha256 02537128a315c8afc9949d7c40510431fbec4bb74c0463794d12383840362c15  registered
proofs/out/P38/usc5-552-doj/queries.cypher                 sha256 156db773c1e21c62f110a3a99d280c0fa66b217b4d614bf36a3247bbc1440105  registered
shows: events 621, args 940; obligatory events with an agent 68; precedes edges 12
shows: same count as the SPARQL query of P37: True (68)
shows: make by agency shall make
shows: state by agency shall separately state
...
```
proofs/out/P39/usc5-552-doj/site/index.html                sha256 820e61732036d0494e33bfbeba89e8b061019deaba4951a3c462f49149f88d6b  registered
proofs/out/P39/usc5-552-doj/site/facts.html                sha256 977568f33992fb43e5640c6edc17fbe4097d418b8c75f9c80d6367d46d05cfbc  registered
...
```
proofs/out/P40/usc5-552-doj/roundtrip.json                 sha256 c7dd5eaa4558c9725afe526cb11feffee61dce89d0baffdcbc3801f7d5e335be  registered
