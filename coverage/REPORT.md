...
## Phase 2: procedural generation at volume, reverse-engineered into the catalogue

### The ask

The catalogue is the model under test. Its claim is that the 38 families and the grammar cover the space of useful constructions and that the families hold across scales. The test that can falsify it needs cases produced with no knowledge of the model, at volume, each one reverse-engineered afterwards: work backwards from what the case requires and see whether the model's chains reconstruct it, where that needs assumptions, where it fails, and whether the same reconstruction holds when the same need appears in another regime. Generating from the model would be steering; reading cases one at a time is waste; mapping the generator's vocabulary once and declaring everything covered would certify the vocabulary, not the cases. Gaps are the product: a case counts as covered only when the framework reconstructs it completely; anything that cannot be reconstructed names a missing family, composition form, axis value, boundary rule, proof record or adapter. The Phase 1 ledger "family fits, no anchoring proof record" is a proof-layer gap under this reading: seventeen entries so far (twelve from iteration 1, five from iteration 2).

### Method

1. **Blind generator.** One zero-context Opus agent receives `coverage/gen/v2_prompt.md`: the user's procedural-generator brief verbatim, plus a wrapper that only fixes machine-readability and reliability (output layout; every relationship and requirement referencing one of the generator's own ingredient ids; regime as a sampling axis; the three real inventories from Phase 1 as seed material; eight construction requirements C1 to C8: the deliverable is a program with fixed command lines and no language-model step inside it, byte-identical reruns from the recorded seed, a runtime budget, provenance on every ingredient, a vocabulary floor of 40 per axis, diversity floors per regime, ISIC section, FORD field, relationship kind, cross-regime cases and regime-by-domain cells, compatibility stored as data the checker re-derives from, and full expansion). The wrapper carries no family, operation, composition, axis, variation-rule or proof identifiers (checked mechanically). Batch size for the first run: 5,000, derived from the 189 regime-by-domain cells needing at least 15 cases each with room for weighting toward underrepresented combinations; it is not a fixed target.
2. **Reliability gate** (`coverage/gen/gate.py`, operator side, mechanical): G1 determinism (same seed reproduces the batch byte for byte in separate processes, another seed differs); G2 checker (the batch passes, the generator's own broken examples are rejected, and operator-made mutations of admitted cases are rejected: dangling reference, no acceptance conditions, no requirements, no inputs); G3 references and required fields, every case expanded; G4 exact and structural duplicates; G5 diversity floors scaled to the batch; G6 leak scan for catalogue identifiers and phrases; G7 provenance share; G8 runtime. A failed gate returns to the agent as a new generation version; the operator edits neither vocabulary nor cases. Smoke-tested on a mock build before use.
3. **Model encoding** (`coverage/model/encode.py` to `catalogue.json`): a mechanical parse of the master's labelled lines, tables and fenced chains, never shown to the generator. Counts match the master (38 families, 18 operations, 14 compositions, 18 axes, 14 boundary fields, 18 variation rules, 32 claims, 12 chains, 65 libraries, 349 proofs; 115 port roles). Facts about the master surfaced by the parse: 98 theoretical records (P203 to P300) carry no hardening block; seven families (F12, F22, F27, F28, F35, F36, F38) have no proof record naming them as primary family; 58 input roles are supplied from outside every family (operating-model inputs) and 37 output roles are consumed by no family (terminal products).
4. **Derivation engine** (`coverage/engine/derive.lp`, clingo 5.7.1, the master's L09 pin; runner `engine.py`): for each case, each deliverable must be realized by one output role of one family; families may join the chain only to feed an input role of a chain family; input roles resolve from case inputs or from chain producers, and unresolved ones are recorded as open inputs (the model's own question mechanism, not a gap); every case input must be consumed; every relationship maps to a composition form with both endpoints explained; every requirement maps to an operation class performed by a chain family; every global constraint maps to a boundary field, a variation rule or the R14 global-invariant claim; every acceptance condition maps to an assurance-claim value with a qualifying operation (O16) in the chain; every situation ingredient maps to an axis value; variation rules follow from mapped axis values through `feature_triggers.json` (the operator's reading of the V table, fixed once) or from a multi-family chain (V12); each chain family needs an anchoring proof record bridged from a case ingredient. Costs: unexplained elements first, then new bridge atoms over accepted ones, then proof gaps, open inputs and chain size. Verdicts: COVERED (nothing unexplained, every chain family anchored), PARTIAL (nothing unexplained, a proof-layer gap), UNEXPLAINED (a named case element found no target), NO-WITNESS (no model within budget).
5. **Bridge, abduced not assumed.** The generator's terms and the model's roles are different vocabularies. Candidates come from token overlap between an ingredient's id and meaning and the model's role names, family texts, operation names, composition forms, boundary fields, variation features, axis values and proof titles, ranked, with generic roles that nearly every family carries penalized. In propose mode the solver may use candidates at a cost by rank and the run emits, per position and ingredient, the candidates, the solver's choices and the cases forcing a decision. The operator accepts or rejects atoms with a reason (`bridge.json`, with provenance); rejected or absent targets are gap candidates. Decisions are per ingredient, so their number is bounded by the vocabulary, not by the case count.
6. **Operator's part, and only this:** bridge-atom judgments with reasons; classification of each unexplained case as a break, a generator defect or a missing bridge; reading of the regime-invariance report, where the same semantic combination instantiated in several regimes must close under the same families or be recorded as a strain.
7. **Outputs per generation version, committed:** per-case derivations, model coverage computed from closed derivations including the parts nothing reaches, the bridge table with provenance, the unexplained list with verdicts, the regime-invariance report and the generator's own sparse-area counts. Volume: the first batch builds the bridge, then 25,000 with the generator expanding its own vocabulary where its counts are sparse (a new generation version from the blind agent, never from the operator). The number to watch is new bridge atoms per thousand cases; it should fall to zero. If it does not, that is the finding.

### Files (Phase 2)

- `coverage/gen/v1_prompt.md`, `coverage/gen/v2_prompt.md`: the generator briefs (v2 is the one run).
- `coverage/gen/gate.py`: the reliability gate.
- `coverage/model/encode.py`, `coverage/model/catalogue.json`, `coverage/model/encode_report.txt`: the model encoding.
- `coverage/engine/derive.lp`, `coverage/engine/feature_triggers.json`, `coverage/engine/engine.py`: the derivation engine.
- Per version: `coverage/runs/<version>/` with the gate report, canonical cases, proposals, bridge, derivations, summary, gaps and invariance.
...
**Superseded.** The verdict table below counts cases this evaluator could not reconstruct under a one-level supplier bound and a first-pass bridge; it is not a count of framework gaps. See "Correction after review" below for the checks and the corrected tables.
